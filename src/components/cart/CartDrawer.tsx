  ...
  return (
    <div className={`fixed inset-0 z-50 ${isOpen ? "" : "pointer-events-none"}`} aria-hidden={!isOpen}>
      ...
      <aside
        ...
        className={`absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-paper shadow-2xl transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <header className="flex items-center justify-between border-b border-sand px-6 py-5">
          <h2 className="font-display text-2xl font-semibold text-forest">Your basket</h2>
        ...
        {items.length === 0 ? (
          ...
          <>
            <ul className="flex-1 divide-y divide-sand overflow-y-auto px-6">
              {items.map((item) => (
                <li key={item.slug} className="flex gap-4 py-4">
                  <ProductImage src={item.image} name={item.name} category={item.category} className="h-20 w-20 shrink-0 rounded-xl border border-sand" sizes="80px" label={false} />
            ...
            <footer className="space-y-3 border-t border-sand bg-cream px-6 py-5">
              <div className="flex justify-between text-lg">
                <span>Subtotal</span>
                <span className="font-display font-semibold">{cedis(subtotal)}</span>
              </div>
              <p className="text-xs text-muted">Pay with Mobile Money or cash at pickup. Delivery fee confirmed on WhatsApp.</p>
              ...
              <a href={whatsappLink(waMessage)} target="_blank" rel="noopener noreferrer" className="block rounded-full border border-forest py-3 text-center font-semibold text-forest hover:bg-sand">
